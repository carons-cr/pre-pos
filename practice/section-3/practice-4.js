...
function createUpdatedCollection(collectionA, objectB) {
  ...
  for (var i = 1; i < collectionA.length; i++){
    ...
    if (collectionA[i] == elementsArray[elementsArray.length-1][0]) {
    ...
    } else if(collectionA[i].indexOf('-')>0){
      var numStr="";
      var num=0;
      for(var j1=0;j1<collectionA[i].length;j1++){
        if(collectionA[i].charAt(j1)>=0&&collectionA[i].charAt(j1)<10){
          numStr+=collectionA[i].charAt(j1);
        }
      }
      num=parseInt(numStr);
      for (var j2 = elementsArray.length - 1; j2 >= 0; j2--) {
        if (collectionA[i].charAt(0) == elementsArray[j2][0]) {
          for(var k=0;k<num;k++){
            elementsArray[j2][elementsArray[j2].length] = collectionA[i].charAt(0);
          }
          isInPreArr = 1;
        }
      }
      if (isInPreArr != 1) {
        ...
        for(var k=0;k<num;k++){
          elementsArray[length][elementsArray[length].length] = collectionA[i].charAt(0);
        }
      }
    } else {
